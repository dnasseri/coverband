...
module Coverband
  class MemoryCacheStoreTest < Test::Unit::TestCase
    def setup
      ...
      @store.checksum_generator = DummyChecksumGenerator.new('fakechecksum')
      @store.clear!
    ...
    test 'it initializes cache with what is in store' do
      ...
      existing_store = Coverband::Adapters::RedisStore.new(@redis)
      existing_store.checksum_generator = DummyChecksumGenerator.new('fakechecksum')
